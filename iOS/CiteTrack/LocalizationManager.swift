...
public class LocalizationManager: ObservableObject {
    ...
    private func addSupplementalKeys() {
        ...
        en["venue_type_book"] = en["venue_type_book"] ?? "Book series"
        // Per-paper analysis + clickable drill-downs
        en["analysis_per_paper_title"] = en["analysis_per_paper_title"] ?? "Per-paper analysis"
        en["analysis_per_paper_subtitle"] = en["analysis_per_paper_subtitle"] ?? "Tap a paper to analyze who cites it — directions, notable citers, venues, and more."
        en["analysis_per_paper_empty"] = en["analysis_per_paper_empty"] ?? "Analyze papers above first; each one can then be analyzed on its own."
        en["analysis_n_citing_papers"] = en["analysis_n_citing_papers"] ?? "%d citing papers"
        en["analysis_direction_paper_count"] = en["analysis_direction_paper_count"] ?? "%d papers in this direction"
        en["analysis_example_papers"] = en["analysis_example_papers"] ?? "Example papers"
        en["analysis_hindex"] = en["analysis_hindex"] ?? "h-index"
        en["analysis_citations_label"] = en["analysis_citations_label"] ?? "Citations"
        en["analysis_works_label"] = en["analysis_works_label"] ?? "Works"
        en["analysis_authors_label"] = en["analysis_authors_label"] ?? "Authors"
        en["analysis_year_label"] = en["analysis_year_label"] ?? "Year"
        en["analysis_view_openalex"] = en["analysis_view_openalex"] ?? "View on OpenAlex"
        en["analysis_citer_papers"] = en["analysis_citer_papers"] ?? "%d of their papers cite you"
        en["analysis_institution_authors"] = en["analysis_institution_authors"] ?? "Authors who cited you"
        en["analysis_institution_no_authors"] = en["analysis_institution_no_authors"] ?? "Author details unavailable for this institution."
        en["analysis_paper_detail_title"] = en["analysis_paper_detail_title"] ?? "Citing paper"
        ...
        en["analysis_stop"] = en["analysis_stop"] ?? "Stop"
        en["analysis_cancelled"] = en["analysis_cancelled"] ?? "Analysis cancelled."
        en["analysis_delete"] = en["analysis_delete"] ?? "Delete"
        en["analysis_delete_confirm"] = en["analysis_delete_confirm"] ?? "Delete this analysis and its impact score? This cannot be undone."
        // "Score your impact" module (formerly the 海优 mock review)
        en["score_impact_title"] = en["score_impact_title"] ?? "Score your impact"
        en["score_impact_entry_subtitle"] = en["score_impact_entry_subtitle"] ?? "Rate your academic impact from your citations"
        en["score_impact_intro"] = en["score_impact_intro"] ?? "Scoring your academic impact from your citation analysis across several dimensions…"
        en["score_impact_rescore"] = en["score_impact_rescore"] ?? "Re-score"
        en["score_impact_running"] = en["score_impact_running"] ?? "Scoring…"
        en["score_impact_dimensions"] = en["score_impact_dimensions"] ?? "Impact dimensions"
        en["score_impact_overall"] = en["score_impact_overall"] ?? "Overall & top suggestions"
        en["score_impact_tier_top"] = en["score_impact_tier_top"] ?? "Outstanding impact (≥85)"
        en["score_impact_tier_strong"] = en["score_impact_tier_strong"] ?? "Strong impact (70–84)"
        en["score_impact_tier_dev"] = en["score_impact_tier_dev"] ?? "Developing impact (<70)"
        en["cancel"] = en["cancel"] ?? "Cancel"
        en["analysis_cost_hint"] = en["analysis_cost_hint"] ?? "≈ $0.002 per run · ~24 free runs/day"
        en["analysis_step_enrich"] = en["analysis_step_enrich"] ?? "Enriching via OpenAlex…"
        en["analysis_step_cluster"] = en["analysis_step_cluster"] ?? "Clustering research directions…"
        en["analysis_step_rank"] = en["analysis_step_rank"] ?? "Ranking top-cited papers…"
        en["analysis_step_inst"] = en["analysis_step_inst"] ?? "Aggregating institutions…"
        en["analysis_step_notable"] = en["analysis_step_notable"] ?? "Finding notable citers…"
        en["insights_clear_all"] = en["insights_clear_all"] ?? "Clear all"
        en["insights_clear_all_confirm"] = en["insights_clear_all_confirm"] ?? "Clear all analyzed publications? Their 海优 scores will be deleted too."
        // Citation Insights page (CitationInsightsView + PublicationCitationDetailView)
        en["insights_title"] = en["insights_title"] ?? "Citation Insights"
        en["insights_signin_subtitle"] = en["insights_signin_subtitle"] ?? "See how others cite your work"
        en["insights_signin_desc"] = en["insights_signin_desc"] ?? "Discover how researchers worldwide cite your work — see the exact passages where your papers are referenced."
        en["whocited_signin_subtitle"] = en["whocited_signin_subtitle"] ?? "Sign in to see who cites you"
        en["whocited_signin_desc"] = en["whocited_signin_desc"] ?? "AI-powered analysis of who cites each of your papers is available after you sign in with Google."
        en["insights_select_scholar"] = en["insights_select_scholar"] ?? "Select Scholar"
        en["insights_add_scholar_first"] = en["insights_add_scholar_first"] ?? "Add a scholar in the Dashboard first."
        en["insights_citations_suffix"] = en["insights_citations_suffix"] ?? "%d citations"
        en["insights_select_publications"] = en["insights_select_publications"] ?? "Select Publications"
        en["insights_loading_pubs"] = en["insights_loading_pubs"] ?? "Loading publications…"
        en["insights_retry"] = en["insights_retry"] ?? "Retry"
        en["insights_preparing"] = en["insights_preparing"] ?? "Preparing…"
        en["insights_select_scholar_hint"] = en["insights_select_scholar_hint"] ?? "Select a scholar above to load publications"
        en["insights_choose"] = en["insights_choose"] ?? "Choose"
        en["insights_analyze"] = en["insights_analyze"] ?? "Analyze"
        en["insights_analyze_n"] = en["insights_analyze_n"] ?? "Analyze (%d)"
        en["insights_all_analyzed"] = en["insights_all_analyzed"] ?? "All Analyzed"
        en["insights_selected_count"] = en["insights_selected_count"] ?? "%d/%d selected"
        en["insights_n_analyzed"] = en["insights_n_analyzed"] ?? "%d publications analyzed"
        en["insights_done"] = en["insights_done"] ?? "Done"
        en["insights_select_all"] = en["insights_select_all"] ?? "Select All"
        en["insights_deselect_all"] = en["insights_deselect_all"] ?? "Deselect All"
        en["insights_all"] = en["insights_all"] ?? "All"
        en["insights_publications_count"] = en["insights_publications_count"] ?? "Publications (%d)"
        en["insights_cited_by"] = en["insights_cited_by"] ?? "Cited by %d"
        en["insights_analyzing"] = en["insights_analyzing"] ?? "Analyzing citations…"
        en["insights_contexts_found"] = en["insights_contexts_found"] ?? "%d citation contexts found so far"
        en["insights_done_count"] = en["insights_done_count"] ?? "%d done"
        en["insights_notfound_count"] = en["insights_notfound_count"] ?? "%d not found"
        en["insights_papers_analyzed"] = en["insights_papers_analyzed"] ?? "Papers Analyzed"
        en["insights_citing_papers"] = en["insights_citing_papers"] ?? "Citing Papers"
        en["insights_contexts"] = en["insights_contexts"] ?? "Contexts"
        en["insights_not_on_ss"] = en["insights_not_on_ss"] ?? "Not on Semantic Scholar"
        en["insights_rate_limited"] = en["insights_rate_limited"] ?? "Rate limited"
        en["insights_error"] = en["insights_error"] ?? "Error"
        en["insights_n_citing"] = en["insights_n_citing"] ?? "%d citing"
        en["insights_n_quotes"] = en["insights_n_quotes"] ?? "%d quotes"
        en["insights_no_data"] = en["insights_no_data"] ?? "No citation data found."
        en["insights_no_data_desc"] = en["insights_no_data_desc"] ?? "Selected publications may not be indexed on Semantic Scholar, or have no citations with context."
        en["insights_your_publications"] = en["insights_your_publications"] ?? "Your Publications (%d)"
        en["insights_issues"] = en["insights_issues"] ?? "Issues"
        en["insights_delete"] = en["insights_delete"] ?? "Delete"
        en["insights_with_quotes"] = en["insights_with_quotes"] ?? "With Quotes"
        en["insights_citing_count"] = en["insights_citing_count"] ?? "Citing Papers (%d)"
        en["insights_no_citing"] = en["insights_no_citing"] ?? "No citing papers found."
        en["insights_no_citing_desc"] = en["insights_no_citing_desc"] ?? "This paper may have citations that Semantic Scholar hasn't indexed yet."
        en["insights_sort_by"] = en["insights_sort_by"] ?? "Sort by"
        en["insights_ctx_count"] = en["insights_ctx_count"] ?? "%d/%d papers have citation context"
        en["insights_ctx_explainer"] = en["insights_ctx_explainer"] ?? "Papers without context: Semantic Scholar only extracts verbatim quotes from open-access full texts. Paywalled or non-indexed papers won't have citation context."
        en["insights_citations_title"] = en["insights_citations_title"] ?? "Citations"
        en["insights_no_fulltext"] = en["insights_no_fulltext"] ?? "No full text access"
        en["insights_copy"] = en["insights_copy"] ?? "Copy"
        en["insights_context_i_of_n"] = en["insights_context_i_of_n"] ?? "Context %d of %d"
        en["insights_not_found_title"] = en["insights_not_found_title"] ?? "Not Found on Semantic Scholar"
        en["insights_rate_limited_title"] = en["insights_rate_limited_title"] ?? "Rate Limited"
        en["insights_fetch_error_title"] = en["insights_fetch_error_title"] ?? "Fetch Error"
        en["insights_ss_notfound_msg"] = en["insights_ss_notfound_msg"] ?? "This paper was not found on Semantic Scholar. It may not be indexed yet."
        en["insights_ss_ratelimit_msg"] = en["insights_ss_ratelimit_msg"] ?? "Request was rate-limited. Try again later."
        en["insights_ss_error_msg"] = en["insights_ss_error_msg"] ?? "An error occurred while fetching citation data."
        en["insights_sort_newest"] = en["insights_sort_newest"] ?? "Newest"
        en["insights_sort_oldest"] = en["insights_sort_oldest"] ?? "Oldest"
        en["insights_sort_quotes"] = en["insights_sort_quotes"] ?? "Most quotes"
        en["role_first"] = en["role_first"] ?? "First Author"
        en["role_co_first"] = en["role_co_first"] ?? "Co-First Author"
        en["role_corresponding"] = en["role_corresponding"] ?? "Corresponding"
        en["role_middle"] = en["role_middle"] ?? "Middle Author"
        en["role_unknown"] = en["role_unknown"] ?? "Unknown"
        en["intent_methodology"] = en["intent_methodology"] ?? "Methodology"
        en["intent_background"] = en["intent_background"] ?? "Background"
        en["intent_result"] = en["intent_result"] ?? "Result"
        en["intent_extends"] = en["intent_extends"] ?? "Extends"
        en["intent_other"] = en["intent_other"] ?? "Other"
        ...
        en["haiyou_title"] = en["haiyou_title"] ?? "Haiyou Mock Score"
        en["haiyou_intro"] = en["haiyou_intro"] ?? "Simulates the NSFC Overseas Excellent Young Scientists 5-dimension review (15+30+20+20+15=100) from your citation analysis. Fill in CV and return plan to improve dimensions 1 & 5."
        en["haiyou_edit_inputs"] = en["haiyou_edit_inputs"] ?? "Edit CV / Return Plan / Representative Papers"
        en["haiyou_run"] = en["haiyou_run"] ?? "Run mock scoring"
        en["haiyou_rescore"] = en["haiyou_rescore"] ?? "Re-score"
        en["haiyou_running"] = en["haiyou_running"] ?? "Reviewers scoring…"
        en["haiyou_dimensions"] = en["haiyou_dimensions"] ?? "Five Dimensions"
        en["haiyou_overall"] = en["haiyou_overall"] ?? "Overall Assessment & Priority Suggestions"
        en["haiyou_priority"] = en["haiyou_priority"] ?? "Priority funding (≥85)"
        en["haiyou_approved"] = en["haiyou_approved"] ?? "Approved (70–84)"
        en["haiyou_rejected"] = en["haiyou_rejected"] ?? "Below funding line (<70)"
        en["haiyou_incomplete"] = en["haiyou_incomplete"] ?? "No CV / return plan provided; dimensions 1 & 5 are low-confidence estimates. Fill them in and re-score."
        en["haiyou_edit_inputs"] = en["haiyou_edit_inputs"] ?? "Fill CV / Return plan / Representative papers"
        en["haiyou_cv"] = en["haiyou_cv"] ?? "Education & work history (Dimension 1)"
        en["haiyou_cv_hint"] = en["haiyou_cv_hint"] ?? "Paste education, postdoc/employers, mentors, key projects."
        en["haiyou_plan"] = en["haiyou_plan"] ?? "Return plan & host support (Dimension 5)"
        en["haiyou_plan_hint"] = en["haiyou_plan_hint"] ?? "Key scientific problem, work plan, quantified host support (lab area/equipment/funding/team)."
        en["haiyou_reppapers"] = en["haiyou_reppapers"] ?? "Representative papers (Dimension 2, up to 5)"
        en["haiyou_paper_title"] = en["haiyou_paper_title"] ?? "Paper title"
        en["haiyou_paper_journal"] = en["haiyou_paper_journal"] ?? "Journal"
        en["haiyou_paper_role"] = en["haiyou_paper_role"] ?? "Authorship (e.g. sole first)"
        en["haiyou_paper_cites"] = en["haiyou_paper_cites"] ?? "Citations"
        en["haiyou_paper_esi"] = en["haiyou_paper_esi"] ?? "ESI Highly Cited"
        en["haiyou_add_paper"] = en["haiyou_add_paper"] ?? "Add representative paper"
        en["haiyou_inputs_title"] = en["haiyou_inputs_title"] ?? "Supplementary Materials"
        en["done"] = en["done"] ?? "Done"
        // Feedback
        en["feedback_section"] = en["feedback_section"] ?? "Feedback"
        en["feedback_send"] = en["feedback_send"] ?? "Send Feedback"
        en["feedback_category"] = en["feedback_category"] ?? "What's this about?"
        en["feedback_idea"] = en["feedback_idea"] ?? "Idea"
        en["feedback_bug"] = en["feedback_bug"] ?? "Bug"
        en["feedback_other"] = en["feedback_other"] ?? "Other"
        en["feedback_message"] = en["feedback_message"] ?? "Your message"
        en["feedback_placeholder"] = en["feedback_placeholder"] ?? "What would you like us to know?"
        en["feedback_email_optional"] = en["feedback_email_optional"] ?? "Your email (optional)"
        ...
        en["open_in_browser"] = en["open_in_browser"] ?? "Open in browser"
        en["whocite_no_unread"] = en["whocite_no_unread"] ?? "No unread cited publications"
        en["whocite_cited_pubs"] = en["whocite_cited_pubs"] ?? "Cited Publications"
        en["whocite_n_citations"] = en["whocite_n_citations"] ?? "%d citations"
        en["whocite_scholar_cited"] = en["whocite_scholar_cited"] ?? "%@'s cited publications"
        en["whocite_citing_n"] = en["whocite_citing_n"] ?? "Citing Articles (%d)"
        en["whocite_article_detail"] = en["whocite_article_detail"] ?? "Article Detail"
        en["et_al"] = en["et_al"] ?? "et al."
        // Citation context (verbatim-quote) view
        en["ctx_signin_title"] = en["ctx_signin_title"] ?? "Sign in to see citation context"
        en["ctx_signin_desc"] = en["ctx_signin_desc"] ?? "Discover the exact text where this paper cites your work."
        en["ctx_searching"] = en["ctx_searching"] ?? "Searching Semantic Scholar…"
        en["ctx_unavailable"] = en["ctx_unavailable"] ?? "Context not available"
        en["ctx_not_indexed"] = en["ctx_not_indexed"] ?? "This paper may not yet be indexed in %@."
        en["ctx_failed"] = en["ctx_failed"] ?? "Failed to load"
        en["ctx_ratelimit_msg"] = en["ctx_ratelimit_msg"] ?? "Semantic Scholar rate limit reached. Please try again in a minute."
        en["ctx_source"] = en["ctx_source"] ?? "Source: %@"
        // Google sign-in gate
        en["signin_headline"] = en["signin_headline"] ?? "See How You're Cited"
        en["signin_subtitle"] = en["signin_subtitle"] ?? "Sign in with Google to unlock verbatim citation context — the exact sentences where your work is referenced."
        en["signin_maybe_later"] = en["signin_maybe_later"] ?? "Maybe Later"
        en["signin_continue_google"] = en["signin_continue_google"] ?? "Continue with Google"
        ...
        zh["venue_type_book"] = zh["venue_type_book"] ?? "丛书"
        // 逐篇分析 + 可点击下钻
        zh["analysis_per_paper_title"] = zh["analysis_per_paper_title"] ?? "逐篇分析"
        zh["analysis_per_paper_subtitle"] = zh["analysis_per_paper_subtitle"] ?? "点击任一篇论文，单独分析它的引用情况——研究方向、知名引用者、刊物等。"
        zh["analysis_per_paper_empty"] = zh["analysis_per_paper_empty"] ?? "请先在上方分析论文；之后即可对每一篇单独分析。"
        zh["analysis_n_citing_papers"] = zh["analysis_n_citing_papers"] ?? "%d 篇引用论文"
        zh["analysis_direction_paper_count"] = zh["analysis_direction_paper_count"] ?? "该方向下 %d 篇论文"
        zh["analysis_example_papers"] = zh["analysis_example_papers"] ?? "代表性论文"
        zh["analysis_hindex"] = zh["analysis_hindex"] ?? "h 指数"
        zh["analysis_citations_label"] = zh["analysis_citations_label"] ?? "总引用"
        zh["analysis_works_label"] = zh["analysis_works_label"] ?? "论文数"
        zh["analysis_authors_label"] = zh["analysis_authors_label"] ?? "作者数"
        zh["analysis_year_label"] = zh["analysis_year_label"] ?? "年份"
        zh["analysis_view_openalex"] = zh["analysis_view_openalex"] ?? "在 OpenAlex 查看"
        zh["analysis_citer_papers"] = zh["analysis_citer_papers"] ?? "其 %d 篇论文引用了你"
        zh["analysis_institution_authors"] = zh["analysis_institution_authors"] ?? "引用你的作者"
        zh["analysis_institution_no_authors"] = zh["analysis_institution_no_authors"] ?? "暂无该机构的作者详情。"
        zh["analysis_paper_detail_title"] = zh["analysis_paper_detail_title"] ?? "引用论文"
        ...
        zh["analysis_stop"] = zh["analysis_stop"] ?? "停止"
        zh["analysis_cancelled"] = zh["analysis_cancelled"] ?? "分析已取消。"
        zh["analysis_delete"] = zh["analysis_delete"] ?? "删除"
        zh["analysis_delete_confirm"] = zh["analysis_delete_confirm"] ?? "删除该分析及其影响力评分？此操作不可撤销。"
        // “影响力评分”模块（原“海优模拟评审”）
        zh["score_impact_title"] = zh["score_impact_title"] ?? "影响力评分"
        zh["score_impact_entry_subtitle"] = zh["score_impact_entry_subtitle"] ?? "用引用数据为你的学术影响力打分"
        zh["score_impact_intro"] = zh["score_impact_intro"] ?? "正在基于你的引用分析，从多个维度为你的学术影响力打分…"
        zh["score_impact_rescore"] = zh["score_impact_rescore"] ?? "重新评分"
        zh["score_impact_running"] = zh["score_impact_running"] ?? "评分中…"
        zh["score_impact_dimensions"] = zh["score_impact_dimensions"] ?? "影响力维度"
        zh["score_impact_overall"] = zh["score_impact_overall"] ?? "整体评价与建议"
        zh["score_impact_tier_top"] = zh["score_impact_tier_top"] ?? "影响力突出 (≥85)"
        zh["score_impact_tier_strong"] = zh["score_impact_tier_strong"] ?? "影响力较强 (70–84)"
        zh["score_impact_tier_dev"] = zh["score_impact_tier_dev"] ?? "影响力成长中 (<70)"
        zh["cancel"] = zh["cancel"] ?? "取消"
        zh["analysis_cost_hint"] = zh["analysis_cost_hint"] ?? "每次约 $0.002 · 每天约 24 次免费"
        zh["analysis_step_enrich"] = zh["analysis_step_enrich"] ?? "正在通过 OpenAlex 富化…"
        zh["analysis_step_cluster"] = zh["analysis_step_cluster"] ?? "正在聚类研究方向…"
        zh["analysis_step_rank"] = zh["analysis_step_rank"] ?? "正在排序高引论文…"
        zh["analysis_step_inst"] = zh["analysis_step_inst"] ?? "正在聚合引用机构…"
        zh["analysis_step_notable"] = zh["analysis_step_notable"] ?? "正在查找知名引用者…"
        zh["insights_clear_all"] = zh["insights_clear_all"] ?? "清空"
        zh["insights_clear_all_confirm"] = zh["insights_clear_all_confirm"] ?? "清空所有已分析的论文？对应的海优评分也会一并删除。"
        // 引用洞察页面（CitationInsightsView + PublicationCitationDetailView）
        zh["insights_title"] = zh["insights_title"] ?? "引用洞察"
        zh["insights_signin_subtitle"] = zh["insights_signin_subtitle"] ?? "看看别人是怎么引用你的"
        zh["insights_signin_desc"] = zh["insights_signin_desc"] ?? "了解全球研究者如何引用你的成果——查看你的论文被引用的具体段落。"
        zh["whocited_signin_subtitle"] = zh["whocited_signin_subtitle"] ?? "登录后查看谁在引用你"
        zh["whocited_signin_desc"] = zh["whocited_signin_desc"] ?? "使用 Google 登录后，即可查看 AI 对每篇论文「谁引用了你」的智能分析。"
        zh["insights_select_scholar"] = zh["insights_select_scholar"] ?? "选择学者"
        zh["insights_add_scholar_first"] = zh["insights_add_scholar_first"] ?? "请先在仪表板添加学者。"
        zh["insights_citations_suffix"] = zh["insights_citations_suffix"] ?? "%d 次引用"
        zh["insights_select_publications"] = zh["insights_select_publications"] ?? "选择论文"
        zh["insights_loading_pubs"] = zh["insights_loading_pubs"] ?? "正在加载论文…"
        zh["insights_retry"] = zh["insights_retry"] ?? "重试"
        zh["insights_preparing"] = zh["insights_preparing"] ?? "正在准备…"
        zh["insights_select_scholar_hint"] = zh["insights_select_scholar_hint"] ?? "请在上方选择学者以加载论文"
        zh["insights_choose"] = zh["insights_choose"] ?? "选择"
        zh["insights_analyze"] = zh["insights_analyze"] ?? "分析"
        zh["insights_analyze_n"] = zh["insights_analyze_n"] ?? "分析（%d）"
        zh["insights_all_analyzed"] = zh["insights_all_analyzed"] ?? "已全部分析"
        zh["insights_selected_count"] = zh["insights_selected_count"] ?? "已选 %d/%d"
        zh["insights_n_analyzed"] = zh["insights_n_analyzed"] ?? "已分析 %d 篇论文"
        zh["insights_done"] = zh["insights_done"] ?? "完成"
        zh["insights_select_all"] = zh["insights_select_all"] ?? "全选"
        zh["insights_deselect_all"] = zh["insights_deselect_all"] ?? "取消全选"
        zh["insights_all"] = zh["insights_all"] ?? "全部"
        zh["insights_publications_count"] = zh["insights_publications_count"] ?? "论文（%d）"
        zh["insights_cited_by"] = zh["insights_cited_by"] ?? "被引 %d"
        zh["insights_analyzing"] = zh["insights_analyzing"] ?? "正在分析引用…"
        zh["insights_contexts_found"] = zh["insights_contexts_found"] ?? "已找到 %d 条引用上下文"
        zh["insights_done_count"] = zh["insights_done_count"] ?? "%d 已完成"
        zh["insights_notfound_count"] = zh["insights_notfound_count"] ?? "%d 未找到"
        zh["insights_papers_analyzed"] = zh["insights_papers_analyzed"] ?? "已分析论文"
        zh["insights_citing_papers"] = zh["insights_citing_papers"] ?? "引用论文"
        zh["insights_contexts"] = zh["insights_contexts"] ?? "引用片段"
        zh["insights_not_on_ss"] = zh["insights_not_on_ss"] ?? "未收录于 Semantic Scholar"
        zh["insights_rate_limited"] = zh["insights_rate_limited"] ?? "触发限流"
        zh["insights_error"] = zh["insights_error"] ?? "出错"
        zh["insights_n_citing"] = zh["insights_n_citing"] ?? "%d 篇引用"
        zh["insights_n_quotes"] = zh["insights_n_quotes"] ?? "%d 条引文"
        zh["insights_no_data"] = zh["insights_no_data"] ?? "未找到引用数据。"
        zh["insights_no_data_desc"] = zh["insights_no_data_desc"] ?? "所选论文可能未被 Semantic Scholar 收录，或没有带上下文的引用。"
        zh["insights_your_publications"] = zh["insights_your_publications"] ?? "我的论文（%d）"
        zh["insights_issues"] = zh["insights_issues"] ?? "问题"
        zh["insights_delete"] = zh["insights_delete"] ?? "删除"
        zh["insights_with_quotes"] = zh["insights_with_quotes"] ?? "含引文"
        zh["insights_citing_count"] = zh["insights_citing_count"] ?? "引用论文（%d）"
        zh["insights_no_citing"] = zh["insights_no_citing"] ?? "未找到引用论文。"
        zh["insights_no_citing_desc"] = zh["insights_no_citing_desc"] ?? "这篇论文的引用可能尚未被 Semantic Scholar 收录。"
        zh["insights_sort_by"] = zh["insights_sort_by"] ?? "排序"
        zh["insights_ctx_count"] = zh["insights_ctx_count"] ?? "%d/%d 篇有引用上下文"
        zh["insights_ctx_explainer"] = zh["insights_ctx_explainer"] ?? "无上下文的论文：Semantic Scholar 仅从开放获取全文中提取原文引用。付费墙或未收录的论文不会有引用上下文。"
        zh["insights_citations_title"] = zh["insights_citations_title"] ?? "引用"
        zh["insights_no_fulltext"] = zh["insights_no_fulltext"] ?? "无全文访问权限"
        zh["insights_copy"] = zh["insights_copy"] ?? "复制"
        zh["insights_context_i_of_n"] = zh["insights_context_i_of_n"] ?? "上下文 %d / %d"
        zh["insights_not_found_title"] = zh["insights_not_found_title"] ?? "未在 Semantic Scholar 找到"
        zh["insights_rate_limited_title"] = zh["insights_rate_limited_title"] ?? "触发限流"
        zh["insights_fetch_error_title"] = zh["insights_fetch_error_title"] ?? "抓取出错"
        zh["insights_ss_notfound_msg"] = zh["insights_ss_notfound_msg"] ?? "未在 Semantic Scholar 找到这篇论文，可能尚未被收录。"
        zh["insights_ss_ratelimit_msg"] = zh["insights_ss_ratelimit_msg"] ?? "请求被限流，请稍后再试。"
        zh["insights_ss_error_msg"] = zh["insights_ss_error_msg"] ?? "抓取引用数据时出错。"
        zh["insights_sort_newest"] = zh["insights_sort_newest"] ?? "最新"
        zh["insights_sort_oldest"] = zh["insights_sort_oldest"] ?? "最早"
        zh["insights_sort_quotes"] = zh["insights_sort_quotes"] ?? "引文最多"
        zh["role_first"] = zh["role_first"] ?? "第一作者"
        zh["role_co_first"] = zh["role_co_first"] ?? "共同一作"
        zh["role_corresponding"] = zh["role_corresponding"] ?? "通讯作者"
        zh["role_middle"] = zh["role_middle"] ?? "中间作者"
        zh["role_unknown"] = zh["role_unknown"] ?? "未知"
        zh["intent_methodology"] = zh["intent_methodology"] ?? "方法"
        zh["intent_background"] = zh["intent_background"] ?? "背景"
        zh["intent_result"] = zh["intent_result"] ?? "结果"
        zh["intent_extends"] = zh["intent_extends"] ?? "扩展"
        zh["intent_other"] = zh["intent_other"] ?? "其他"
        // 海优 模拟评分
        zh["haiyou_entry_title"] = zh["haiyou_entry_title"] ?? "海优模拟评分"
        zh["haiyou_entry_subtitle"] = zh["haiyou_entry_subtitle"] ?? "按海优五维评审体系给你的影响力打分"
        zh["haiyou_title"] = zh["haiyou_title"] ?? "海优模拟评分"
        zh["haiyou_intro"] = zh["haiyou_intro"] ?? "基于你的引用分析数据，按海优五维评审体系（15+30+20+20+15=100）模拟打分。建议先填写简历与回国设想以提升维度1/5的评分准确度。"
        zh["haiyou_edit_inputs"] = zh["haiyou_edit_inputs"] ?? "填写简历 / 回国设想 / 代表作"
        zh["haiyou_run"] = zh["haiyou_run"] ?? "开始模拟评分"
        zh["haiyou_rescore"] = zh["haiyou_rescore"] ?? "重新评分"
        zh["haiyou_running"] = zh["haiyou_running"] ?? "评审专家打分中…"
        zh["haiyou_dimensions"] = zh["haiyou_dimensions"] ?? "五维评分"
        zh["haiyou_overall"] = zh["haiyou_overall"] ?? "整体评价与优先建议"
        zh["haiyou_priority"] = zh["haiyou_priority"] ?? "优先资助 (≥85)"
        zh["haiyou_approved"] = zh["haiyou_approved"] ?? "同意资助 (70–84)"
        zh["haiyou_rejected"] = zh["haiyou_rejected"] ?? "未达资助线 (<70)"
        zh["haiyou_incomplete"] = zh["haiyou_incomplete"] ?? "未提供简历/回国设想，维度1/5为低置信度估算。填写后重新评分更准确。"
        zh["haiyou_cv"] = zh["haiyou_cv"] ?? "教育与工作经历（维度1）"
        zh["haiyou_cv_hint"] = zh["haiyou_cv_hint"] ?? "粘贴教育经历、博士后/工作单位、师承、重要项目等。"
        zh["haiyou_plan"] = zh["haiyou_plan"] ?? "回国设想与依托单位支持（维度5）"
        zh["haiyou_plan_hint"] = zh["haiyou_plan_hint"] ?? "拟解决的关键科学问题、工作计划、依托单位的量化支持（实验室面积/设备/经费/团队）。"
        zh["haiyou_reppapers"] = zh["haiyou_reppapers"] ?? "代表作（维度2，最多5篇）"
        zh["haiyou_paper_title"] = zh["haiyou_paper_title"] ?? "论文标题"
        zh["haiyou_paper_journal"] = zh["haiyou_paper_journal"] ?? "期刊"
        zh["haiyou_paper_role"] = zh["haiyou_paper_role"] ?? "署名(如:唯一一作)"
        zh["haiyou_paper_cites"] = zh["haiyou_paper_cites"] ?? "他引"
        zh["haiyou_paper_esi"] = zh["haiyou_paper_esi"] ?? "ESI 高被引"
        zh["haiyou_add_paper"] = zh["haiyou_add_paper"] ?? "添加代表作"
        zh["haiyou_inputs_title"] = zh["haiyou_inputs_title"] ?? "补充材料"
        zh["done"] = zh["done"] ?? "完成"
        // 反馈
        zh["feedback_section"] = zh["feedback_section"] ?? "反馈"
        zh["feedback_send"] = zh["feedback_send"] ?? "发送反馈"
        zh["feedback_category"] = zh["feedback_category"] ?? "反馈类型"
        zh["feedback_idea"] = zh["feedback_idea"] ?? "建议"
        zh["feedback_bug"] = zh["feedback_bug"] ?? "问题"
        zh["feedback_other"] = zh["feedback_other"] ?? "其他"
        zh["feedback_message"] = zh["feedback_message"] ?? "你的留言"
        zh["feedback_placeholder"] = zh["feedback_placeholder"] ?? "有什么想让我们知道的？"
        zh["feedback_email_optional"] = zh["feedback_email_optional"] ?? "你的邮箱（选填）"
        ...
        zh["open_in_browser"] = zh["open_in_browser"] ?? "在浏览器中打开"
        zh["whocite_no_unread"] = zh["whocite_no_unread"] ?? "没有未读的被引用论文"
        zh["whocite_cited_pubs"] = zh["whocite_cited_pubs"] ?? "被引用论文"
        zh["whocite_n_citations"] = zh["whocite_n_citations"] ?? "%d 次引用"
        zh["whocite_scholar_cited"] = zh["whocite_scholar_cited"] ?? "%@ 的被引用论文"
        zh["whocite_citing_n"] = zh["whocite_citing_n"] ?? "引用文章（%d）"
        zh["whocite_article_detail"] = zh["whocite_article_detail"] ?? "文章详情"
        zh["et_al"] = zh["et_al"] ?? "等"
        // 引用上下文（原文引用）视图
        zh["ctx_signin_title"] = zh["ctx_signin_title"] ?? "登录后查看引用上下文"
        zh["ctx_signin_desc"] = zh["ctx_signin_desc"] ?? "看看这篇论文具体在哪段文字里引用了你的工作。"
        zh["ctx_searching"] = zh["ctx_searching"] ?? "正在检索 Semantic Scholar…"
        zh["ctx_unavailable"] = zh["ctx_unavailable"] ?? "暂无引用上下文"
        zh["ctx_not_indexed"] = zh["ctx_not_indexed"] ?? "这篇论文可能尚未被 %@ 收录。"
        zh["ctx_failed"] = zh["ctx_failed"] ?? "加载失败"
        zh["ctx_ratelimit_msg"] = zh["ctx_ratelimit_msg"] ?? "Semantic Scholar 触发限流，请一分钟后再试。"
        zh["ctx_source"] = zh["ctx_source"] ?? "来源：%@"
        // Google 登录引导
        zh["signin_headline"] = zh["signin_headline"] ?? "看看谁引用了你"
        zh["signin_subtitle"] = zh["signin_subtitle"] ?? "用 Google 登录，解锁逐字引用上下文——你的工作被引用的具体句子。"
        zh["signin_maybe_later"] = zh["signin_maybe_later"] ?? "以后再说"
        zh["signin_continue_google"] = zh["signin_continue_google"] ?? "使用 Google 继续"
